...
- LPLPO: `DRAFT -> SUBMITTED -> PIC_VERIFIED -> REVIEWED -> APPROVED -> CLOSED`, dengan dua loop penolakan: `SUBMITTED -> REJECTED_PUSKESMAS` untuk koreksi operator Puskesmas dan `REVIEWED -> REJECTED_PIC` untuk koreksi PIC Gudang. Approval Kepala membuat dokumen Distribution LPLPO, lalu LPLPO ditutup saat distribusi terkait selesai. Selama distribusi hasil approval itu belum selesai, approver masih dapat membatalkan distribusi tersebut dan mengembalikan LPLPO ke `REJECTED_PUSKESMAS` dengan alasan revisi.
