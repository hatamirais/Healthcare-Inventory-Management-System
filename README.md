...
- Pelaporan LPLPO bulanan dan pengajuan permintaan barang secara ad-hoc dari Puskesmas, dengan pembuatan dokumen yang terkunci berurutan dari Januari pada tahun server aktif. Januari diperlakukan sebagai bootstrap stok awal tahunan yang diisi manual, sedangkan bulan berikutnya membawa carry-over `sisa stok` bulan sebelumnya ke `stock_awal` selama dokumen bulan sebelumnya tidak berstatus `REJECTED_PUSKESMAS` atau `REJECTED_PIC`.
