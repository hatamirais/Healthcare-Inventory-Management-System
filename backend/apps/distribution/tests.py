...
class DistributionWorkflowTest(TestCase):
    ...
    def test_borrow_rs_create_page_hides_type_and_approved_quantity(self):
        response = self.client.get(reverse("distribution:borrow_rs_create"))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Buat Pinjam RS")
        self.assertContains(response, "Kuantitas")
        self.assertNotContains(response, "Kuantitas Disetujui")
        self.assertNotContains(response, 'name="distribution_type"', html=False)

    def test_borrow_rs_create_auto_verifies_and_copies_quantity(self):
        response = self.client.post(
            reverse("distribution:borrow_rs_create"),
            {
                "document_number": "",
                "request_date": "2026-03-12",
                "facility": self.rs_facility.pk,
                "program": "",
                "notes": "Pinjam mendesak",
                "assigned_staff": [self.user.pk],
                "items-TOTAL_FORMS": "1",
                "items-INITIAL_FORMS": "0",
                "items-MIN_NUM_FORMS": "0",
        ...
        self.assertEqual(dist.distribution_type, Distribution.DistributionType.BORROW_RS)
        self.assertEqual(dist.status, Distribution.Status.VERIFIED)
        self.assertEqual(dist.verified_by, self.user)
        self.assertEqual(dist.facility, self.rs_facility)
        self.assertEqual(line.quantity_requested, Decimal("12"))
        self.assertEqual(line.quantity_approved, Decimal("12"))

    def test_borrow_rs_list_only_shows_borrow_rs_documents(self):
        borrow_dist = Distribution.objects.create(
            distribution_type=Distribution.DistributionType.BORROW_RS,
            request_date="2026-03-10",
        ...
        normal_dist = Distribution.objects.create(
            distribution_type=Distribution.DistributionType.LPLPO,
            request_date="2026-03-10",
            facility=self.facility,
        ...
        response = self.client.get(reverse("distribution:borrow_rs_list"))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, borrow_dist.document_number)
        self.assertNotContains(response, normal_dist.document_number)

    def test_borrow_rs_detail_rejects_non_borrow_distribution(self):
        normal_dist = Distribution.objects.create(
            distribution_type=Distribution.DistributionType.LPLPO,
            request_date="2026-03-10",
            facility=self.facility,
        ...
        response = self.client.get(
            reverse("distribution:borrow_rs_detail", args=[normal_dist.pk])
        )

        self.assertEqual(response.status_code, 404)

    def test_borrow_rs_detail_shows_record_return_action_when_outstanding_exists(self):
        dist = Distribution.objects.create(
            distribution_type=Distribution.DistributionType.BORROW_RS,
            request_date="2026-03-10",
            facility=self.rs_facility,
        ...
        DistributionItem.objects.create(
            ...
            stock=self.stock,
            issued_batch_lot="BATCH-D01",
            issued_expiry_date="2027-12-31",
            issued_unit_price=Decimal("5000"),
            issued_sumber_dana=self.funding_source,
        )

        response = self.client.get(reverse("distribution:borrow_rs_detail", args=[dist.pk]))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, reverse("receiving:rs_return_from_borrow_create", args=[dist.pk]))
        self.assertContains(response, "Catat Pengembalian")
